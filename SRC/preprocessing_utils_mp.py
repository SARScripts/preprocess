...
class model():
    ...
    def mpUnzipAssembly(self, imagelist, ImageDate, date):
        ...
        if ImageDate.count(date) == 1:
            ...
            outputpath = os.path.join(self.diroutputzip, date, '_assembl.dim')
        ...
        if not os.path.isfile(os.path.join(self.diroutorder, subdiroutimag, imagebasename + '.dim')) or self.overwrite == '1':
            for polariz in input5:
                ...
                for k in range(len(input3)):
                    ...
                    try:
                        p = subprocess.check_output([self.pathgpt, self.gptxml_coreg, '-Pinput1='+input1, '-Pinput2='+input2, '-Pinput3='+input3[k], '-Pinput4='+input4, '-Pinput5='+polariz,'-Ptarget1='+outputfile1,'-Ptarget2='+outputfile2 ,'-Ptarget3='+outputfile3])
                    except:
                        p = ''
    ...
    def run_alignmentlist(self, pairspath, pairsdate):
        ...
        # imagebasename = os.path.join(self.diroutorder, subdiroutimag, self.datemaster + '_' + self.datemaster + '_SLC' + self.sufix + output_sufix)
    ...
    def alignment_ifg (self):
        ...
        if self.calib == '1':
            ...
            self.sufix =''
            outputfiles = imagelist['ImagePreprocess']
